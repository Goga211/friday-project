...
Модель «Пятница» обучается отдельно (русское слово нужно тренировать); путь к ней —
FRIDAY_VOICE_WAKE_MODEL. Без модели грузятся предобученные (это НЕ «Пятница»,
...
log = logging.getLogger("friday.voice.wake")
...
class OpenWakeWordDetector:
    def __init__(self, settings: VoiceSettings) -> None:
        ...
        if settings.wake_model:
            ...
            log.warning(
                "FRIDAY_VOICE_WAKE_MODEL не задан — грузятся предобученные модели "
                "(НЕ «Пятница»). Обучи свою модель и укажи путь."
            )
