...
def build_source(settings: VoiceSettings) -> AudioSource:
    ...
    if settings.audio == "sounddevice":
        from christopher.agents.voice.providers.sounddevice_io import MicSource
...
def build_sink(settings: VoiceSettings) -> AudioSink:
    if settings.audio == "fake":
        return FakeSink()
    if settings.audio == "sounddevice":
        from christopher.agents.voice.providers.sounddevice_io import SpeakerSink
...
def build_recognizer(settings: VoiceSettings) -> SpeechRecognizer:
    ...
    if settings.stt == "yandex":
        from christopher.agents.voice.providers.yandex_stt import YandexSpeechRecognizer
