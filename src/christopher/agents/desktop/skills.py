...
# Дефолтный allowlist для run_command (можно расширить CHRISTOPHER_CMD_ALLOWLIST="a,b,c").
_DEFAULT_ALLOWLIST = (
...
def _allowlist() -> set[str]:
    raw = os.getenv("CHRISTOPHER_CMD_ALLOWLIST")
    if raw:
...
async def screenshot(params: dict[str, Any]) -> dict[str, Any]:
    ...
    if not path:
        path = os.path.join(tempfile.gettempdir(), f"christopher-shot-{int(time.time())}.png")
